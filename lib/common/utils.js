...
var crypto = fcash.crypto;
var encoding = fcash.encoding;
...
Utils.hashMessage = function(text, noReverse) {
  ...
  if (!noReverse) {
    ret = new fcash.encoding.BufferReader(ret).readReverse();
  }
  return ret;
};
...
Utils.translateAddress = function(address, coin) {
  ...
  var origAddress = new Fcash_[origCoin].Address(address);
  var origObj = origAddress.toObject();

  var result = Fcash_[coin].Address.fromObject(origObj)
