...
/**
 * Creates an instance of the Fcash Wallet Service.
 * @constructor
 */
...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  try {
    pubKey = new Fcash.PublicKey.fromString(opts.pubKey);
  } catch (ex) {
...
WalletService.prototype._verifyRequestPubKey = function(requestPubKey, signature, xPubKey) {
  var pub = (new Fcash.HDPublicKey(xPubKey)).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).publicKey;
...
WalletService.prototype.joinWallet = function(opts, cb) {
  ...
  var xPubKey;
  try {
    xPubKey = Fcash.HDPublicKey(opts.xPubKey);
  } catch (ex) {
...
WalletService.prototype._getUtxos = function(coin, addresses, cb) {
  ...
  var networkName = Fcash_[coin].Address(addresses[0]).toObject().network;
...
WalletService.prototype._checkTx = function(txp) {
  ...
  try {
    var fcashTx = txp.getFcashTx();
    fcashError = fcashTx.getSerializationError(serializationOpts);
    if (!fcashError) {
      txp.fee = fcashTx.getFee();
    }
  } catch (ex) {
    self.logw('Error building Fcash transaction', ex);
    return ex;
  }

  if (fcashError instanceof Fcash_[txp.coin].errors.Transaction.FeeError)
    return Errors.INSUFFICIENT_FUNDS_FOR_FEE;

  if (fcashError instanceof Fcash_[txp.coin].errors.Transaction.DustOutputs)
  ...
  function select(utxos, cb) {
    ...
    _.each(smallInputs, function(input, i) {
      ...
      if (netTotal >= txpAmount) {
        ...
        var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, Fcash_[txp.coin].Transaction.DUST_AMOUNT);
...
WalletService.prototype._validateOutputs = function(opts, wallet, cb) {
  var A = Fcash_[wallet.coin].Address;
  var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, Fcash_[wallet.coin].Transaction.DUST_AMOUNT);
    ...
    self.getTx({
      ...
      try {
        if (!txp.sign(self.copayerId, opts.signatures, copayer.xPubKey)) {
          ...
          var raw = txp.getFcashTx().uncheckedSerialize();
 ...
 * @param {String} [opts.provider] - A provider of exchange rates (default 'Fcash').
