...
Address.create = function(opts) {
  ...
  x.coin = opts.coin;
  x.network = Fcash[opts.coin].Address(x.address).toObject().network;
...
Address._deriveAddress = function(scriptType, publicKeyRing, path, m, coin, network) {
  ...
  var publicKeys = _.map(publicKeyRing, function(item) {
    var xpub = new Fcash[coin].HDPublicKey(item.xPubKey);
  ...
  switch (scriptType) {
    case Constants.SCRIPT_TYPES.P2SH:
      fcashAddress = Fcash[coin].Address.createMultisig(publicKeys, m, network);
      break;
    case Constants.SCRIPT_TYPES.P2PKH:
      $.checkState(_.isArray(publicKeys) && publicKeys.length == 1);
      fcashAddress = Fcash[coin].Address.fromPublicKey(publicKeys[0], network);
      break;
  }

  return {
    address: fcashAddress.toString(),
    path: path,
