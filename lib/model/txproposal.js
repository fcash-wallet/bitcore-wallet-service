...
TxProposal.prototype._buildTx = function() {
  ...
  var t = new Fcash[self.coin].Transaction();
  ...
  _.each(self.outputs, function(o) {
    ...
    if (o.script) {
      t.addOutput(new Fcash[self.coin].Transaction.Output({
        script: o.script,
        satoshis: o.amount
      }));
    } else {
  ...
  // Validate actual inputs vs outputs independently of Fcash
...
TxProposal.prototype.getFcashTx = function() {
  var self = this;
  ...
  _.each(sigs, function(x) {
    self._addSignaturesToFcashTx(t, x.signatures, x.xpub);
  });
...
TxProposal.prototype._addSignaturesToFcashTx = function(tx, signatures, xpub) {
  var self = this;

  var fcash = Fcash[self.coin];

  if (signatures.length != this.inputs.length)
    throw new Error('Number of signatures does not match number of inputs');

  var i = 0,
    x = new fcash.HDPublicKey(xpub);

  _.each(signatures, function(signatureHex) {
    var input = self.inputs[i];
    try {
      var signature = fcash.crypto.Signature.fromString(signatureHex);
      var pub = x.deriveChild(self.inputPaths[i]).publicKey;
      var s = {
        inputIndex: i,
        signature: signature,
        sigtype: fcash.crypto.Signature.SIGHASH_ALL | fcash.crypto.Signature.SIGHASH_FORKID,
        publicKey: pub,
      };
...
TxProposal.prototype.sign = function(copayerId, signatures, xpub) {
  try {
    ...
    var tx = this.getFcashTx();
    this._addSignaturesToFcashTx(tx, signatures, xpub);
