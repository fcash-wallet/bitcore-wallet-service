...
[![Build Status](https://img.shields.io/travis/bitpay/fcash-wallet-service.svg?branch=master&style=flat-square)](https://travis-ci.org/bitpay/fcash-wallet-service)
[![Coverage Status](https://coveralls.io/repos/bitpay/fcash-wallet-service/badge.svg?branch=master)](https://coveralls.io/r/bitpay/fcash-wallet-service?branch=master)
...
BWS supports SSL and Clustering. For a detailed guide on installing BWS with extra features see [Installing BWS](https://github.com/fcash-wallet/fcash-wallet-service/blob/master/installation.md). 
 ...
 * A copayer could join the wallet more than once, and there is no mechanism to prevent this. See [wallet](https://github.com/fcash-wallet/fcash-wallet)'s confirm command, for a method for confirming copayers.
...
See [Fcash Wallet Client](https://github.com/fcash-wallet/fcash-wallet-client/blob/master/lib/api.js#L73) for implementation details.
...
Returns:
 * Wallet object. (see [fields on the source code](https://github.com/fcash-wallet/fcash-wallet-service/blob/master/lib/model/wallet.js)).
...
Returns:
 * List of pending TX Proposals. (see [fields on the source code](https://github.com/fcash-wallet/fcash-wallet-service/blob/master/lib/model/txproposal.js))

`/v1/addresses/`: Get Wallet's main addresses (does not include change addresses)

Returns:
 * List of Addresses object: (https://github.com/fcash-wallet/fcash-wallet-service/blob/master/lib/model/address.js)).  This call is mainly provided so the client check this addresses for incoming transactions (using a service like [Insight](https://insight.is)
...
Returns:
 * TX Proposal object. (see [fields on the source code](https://github.com/fcash-wallet/fcash-wallet-service/blob/master/lib/model/txproposal.js)). `.id` is probably needed in this case.


`/v3/addresses/`: Request a new main address from wallet . (creates an address on normal conditions)

Returns:
 * Address object: (https://github.com/fcash-wallet/fcash-wallet-service/blob/master/lib/model/address.js)). Note that `path` is returned so client can derive the address independently and check server's response.
...
Returns:
 * TX Proposal object. (see [fields on the source code](https://github.com/fcash-wallet/fcash-wallet-service/blob/master/lib/model/txproposal.js)). `.status` is probably needed in this case.
